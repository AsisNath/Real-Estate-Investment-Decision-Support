...
- **Automatic policy research is opt-in and fails soft.** `app/policy_research.py` calls the Claude API with live web search when an address has no researched note. It was added on the user's explicit instruction, overriding this file's earlier "no runtime web calls" rule - which had said not to add one without discussing it, and the discussion happened. It requires an `ANTHROPIC_API_KEY` in a git-ignored `.env`; without one, or without internet, or with `NORTHSTAR_DISABLE_AUTO_RESEARCH=1`, the app reverts exactly to the old copy-and-paste flow. The test suite sets that kill switch so `pytest` can never bill anyone.
