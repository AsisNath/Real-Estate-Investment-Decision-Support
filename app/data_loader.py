...
def redfin_market(zip_code: str) -> dict[str, Any] | None:
    """Latest Redfin sale metrics for one ZIP, or None if it is not covered.

    Generated by `scripts/refresh_redfin_markets.py`. A missing file only means
    the refresh has never been run, so the app falls back to the bundled sample
    market data rather than failing outright - the same contract as
    `zip_places()`.

    Redfin publishes sale data only. It answers "what is this worth and which
    way is the market moving", never "what does it rent for".
    """
    try:
        markets = load_json("redfin_markets.json")["zip_markets"]
    except (FileNotFoundError, KeyError):
        return None

    record = markets.get(zip_code.strip())
...
def load_market_context(zip_code: str, state: str) -> dict[str, Any]:
    ...
    elif state in data["state_defaults"]:
    ...
    # Real ZIP-level sale data layers on top of whatever sample record matched.
    # It never overwrites median_rent_estimate: Redfin has no rent figures, and
    # a rent number is the one thing this app must not invent.
    redfin = redfin_market(zip_code)
    if redfin:
        result["redfin"] = redfin
        result["missing_data_flags"] = [
            flag.replace(
                "This section uses",
                "Home values below are real Redfin sale data for this ZIP; the rent estimate uses",
            )
