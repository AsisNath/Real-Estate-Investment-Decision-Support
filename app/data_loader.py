...
def zip_places() -> dict[str, Any]:
    """ZIP -> place lookup, covering every US ZIP code.

    Prefers the bundled `zip_places.json` (about 41,500 ZIPs, regenerated by
    `scripts/refresh_zip_places.py`) so no one has to hand-maintain a ZIP list.
    Falls back to the small hand-written table in `zip_directory.json` if that
    file is missing, so the app still runs rather than failing outright.
    """
    try:
        generated = load_json("zip_places.json")["places"]
    except (FileNotFoundError, KeyError):
        return {
            code: [place["city"], place["state"], place["county"]]
            for code, place in load_json("zip_directory.json")["zip_places"].items()
        }
    return generated
...
def check_location_consistency(city: str, state: str, zip_code: str) -> dict[str, Any]:
    ...
    warnings: list[str] = []
    unverified: list[str] = []
    entry = zip_places().get(digits)
    place = (
        {"city": entry[0], "state": entry[1], "county": entry[2]} if entry else None
    )
